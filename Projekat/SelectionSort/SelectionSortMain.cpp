...
#include "sequentialSelectionSort.cpp"
#include "parallelSelectionSort.cpp"
...
double averageParallelTime(int* original, int* buffer, int n, int threads, int runs = 5) {
    ...
    for (int i = 0; i < runs; ++i) {
        ...
        selectionSortParallel(buffer, n);
...
int main() {
    ...
    std::vector<int> n_values = {1000, 5000, 10000, 20000};  // Po želji dodaj veće
    std::vector<int> thread_values = {1, 2, 4, 8};

    std::ofstream out("SelectionSortResults.csv");
    ...
    out.close();
    std::cout << "\nRezultati su sacuvani u 'SelectionSortResults.csv'\n";
    return 0;
}
